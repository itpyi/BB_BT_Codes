"""BT code LER vs rounds (decoder-based; single-shot disabled).

Builds a BT code from polynomials, generates repeated-syndrome Stim circuits,
and estimates post-decoding logical error rate (LER) vs rounds using BpOsd.

Single-shot meta scrubbing is explicitly disabled to isolate round scaling.

Usage examples
--------------

    # Quick default sweep
    python bt_ler_vs_rounds.py
    ...
    python bt_ler_vs_rounds.py \
        ...
        --csv Data/bt_ler_vs_rounds.csv

    python bt_ler_vs_rounds.py --rounds 1 2 3 4 --p-list 0.006 --shots 5000 --max-errors 50 --plot-per-round bt_per_round.png

Notes
-----
- This script mirrors the serial path in `simulation_generic.py` but forces
  `use_bt_singleshot=False` when building the decoder.
...
def run_bt_rounds_sweep(
    *,
    ...
    use_bt_singleshot: bool = False,
) -> List[Point]:
    code = build_bt_code(a_poly, b_poly, c_poly, l, m, estimate_distance=False)
    ...
    for rounds in rounds_list:
        for p in p_list:
            ...
            # Build decoder with single-shot disabled
            decoder, obs_mat = build_decoder_from_circuit(
                ...
                code=code,
                p=float(p),
                use_bt_singleshot=bool(use_bt_singleshot),
            )
            ...
            print(
                f"BT r={rounds:<3d} p={p:.4g} shots={used:<7d} errors={errors:<5d} LER={ler:.6f} per_round={pr:.6f} time={seconds:.2f}s"
            )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="BT code LER vs rounds (BpOsd, single-shot disabled)")
    ...
    ap.add_argument("--c-poly", type=_parse_poly, default="[[0,0],[1,1]]", help="JSON list of [i,j] for c(x,y)")
    ...
    ap.add_argument(
        "--single-shot",
        action="store_true",
        help="Enable BT single-shot meta scrubbing (disabled by default)",
    )
    args = ap.parse_args()

    seed = None if args.seed is None else int(args.seed)
    points = run_bt_rounds_sweep(
        a_poly=args.a_poly,
        b_poly=args.b_poly,
        c_poly=args.c_poly,
        l=args.l,
        ...
        resume_csv=args.csv,
        use_bt_singleshot=bool(args.single_shot),
    )
    ...
    def _maybe_plot(points: List[Point], out_path: Optional[str], *, mode: str) -> None:
        ...
        for p, pts in sorted(by_p.items()):
            ...
            if mode == "per_round":
                ...
                title = "BT code: per-round LER vs rounds (grouped by p)"
            else:
                ys = [q.errors / max(1, q.shots) for q in pts]
                ylabel = "Logical error rate"
                title = "BT code: LER vs rounds (grouped by p)"
